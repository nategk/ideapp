...
app.goToSession = function(session) {
  ...
  //model.page.redirect('/session/'+session);
  //app.history.back()
  app.history.push('/session/' + session);
  //page.history.push('/session/');
  //page.redirect('/sesseion/');
  //model.redirect('/sesseion/');
}
...
app.fn('session.add', function(e, el, next) {
  ...
  var newSession = model.del('_page.newSession');
  if (!newSession) return;

  // Add data
  newSession.dateCreated = +(new Date);
  newSession.userId = model.get('_session.userId');
  session = model.add('sessions', newSession, function() {
    console.log('created ' + session);
    app.goToSession(session);
  });
});
