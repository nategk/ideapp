...
app.get('/', function(page, model) {
  ...
  session = model.add('sessions', {title: "Ideation Session", dateCreated: +(new Date)}, goToSession);
});
...
app.get('/session/:id', function(page, model, params, next) {
  ...
  session.subscribe(function(err) {
    ...
    var user = model.get('_session.userId');
    console.log("User ID " + user);
    // Query notes that belong to this session
    var notesQuery = model.query('notes', {sessionId: session.get('id')});
